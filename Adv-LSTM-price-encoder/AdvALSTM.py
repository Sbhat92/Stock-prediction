    ...
    def call(self, h):
        a_latent = tf.tanh(
            tf.tensordot(h, self.av_W,
                         axes=1) + self.av_b)
        scores = tf.tensordot(a_latent, self.av_u,
                                     axes=1,
                                     name='scores')
        alphas = tf.nn.softmax(scores, name='alphas')

        con = tf.reduce_sum(
            h * tf.expand_dims(alphas, -1), 1)
...
class AdvALSTM(tf.keras.models.Model):
    def __init__(self, units, epsilon = 1E-3, beta =  5E-2, learning_rate = 1E-2, dropout = None, l2 = None, attention = True, hinge = True, adversarial_training = True, random_perturbations = False):
        ...
        self.random_permutations = random_perturbations
        ...
        if attention:
            ...
            self.model_latent_rep = tf.keras.models.Sequential([
                #tf.keras.layers.Dense(units, activation = "tanh"),
                #GRUAttentionLayer(units, 16, dropout),
                tf.keras.layers.Dropout(dropout),
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            if self.adversarial_training:
                ...
                e_adversary = e + self.epsilon * tf.norm(e, ord = "euclidean", axis= -1, keepdims = True) * tf.math.l2_normalize(perturbations, axis = -1, epsilon=1e-8)
                y_pred_adv = self.model_prediction(e_adversary, training = True)
